...
class VKPublisher:
    ...
    def _upload_photo_to_group(self, vk, temp_path: Path, group_id: int) -> Optional[str]:
        """Загружает фото в группу (основной и альтернативный способ)."""
        try:
            # Основной способ через VkUpload
            upload = VkUpload(vk)
            if group_id < 0:
            ...
            logger.warning(f"Основной способ загрузки фото не сработал: {e}")

        # Альтернативный способ через прямой API
        try:
            # Получаем сервер для загрузки
            upload_url = vk.method('photos.getWallUploadServer', {'group_id': abs(group_id) if group_id < 0 else None})
            if not upload_url:
                logger.error("Не удалось получить upload_url")
                return None
            # Загружаем файл
            files = {'photo': open(str(temp_path), 'rb')}
            resp = requests.post(upload_url['upload_url'], files=files).json()
            if resp.get('photo'):
                save_result = vk.method('photos.saveWallPhoto', {
                    'photo': resp['photo'],
                    'server': resp['server'],
                    'hash': resp['hash'],
                    'group_id': abs(group_id) if group_id < 0 else None
                })
                if save_result:
                    photo = save_result[0]
                    return f"photo{photo['owner_id']}_{photo['id']}"
        except Exception as e:
            logger.error(f"Альтернативный способ загрузки фото не сработал: {e}")
    ...
    def publish(self, post: Post, group: Group) -> PublishResult:
        try:
            ...
            if post.image_bytes:
                ...
                photo_attachment = self._upload_photo_to_group(vk, temp_path, group.group_id)
                if photo_attachment:
                    attachments.append(photo_attachment)
                    logger.info("Фото успешно загружено в группу")
                else:
                    logger.warning("Не удалось загрузить фото в группу")
