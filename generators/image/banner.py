"""generators/image/banner.py – создание баннеров и наложение текста (без ошибок прозрачности)"""
import io
import random
...
class BannerGenerator:
    ...
    def create_banner_from_image(self, image_bytes: bytes, title: str = "", subtitle: str = "", cta: str = "") -> bytes:
        ...
        Использует alpha_composite и convert('RGB') – безопасно для любых форматов.
        """
        # 1) Открываем и конвертируем в RGBA (гарантируем наличие альфа-канала)
        ...
        # 3) Накладываем оверлей через alpha_composite (безопасно)
        ...
        # 5) Конвертируем в RGB (убираем альфа-канал) и сохраняем
    ...
    def _draw_overlay(self, draw):
        # Этот метод используется только в create_banner, не в create_banner_from_image
