"""generators/image/banner.py – создание баннеров и наложение текста (безопасная версия)"""
import io
import random
...
class BannerGenerator:
    ...
    def create_banner(self, title: str = "", subtitle: str = "", cta: str = "", category: str = "general", is_announce: bool = False) -> bytes:
        ...
        # 2) Создаём оверлей (полупрозрачный чёрный снизу)
        ...
        # 3) Накладываем оверлей на img (через RGBA)
    ...
    def create_banner_from_image(self, image_bytes: bytes, title: str = "", subtitle: str = "", cta: str = "") -> bytes:
        """Накладывает текст на изображение (безопасно)"""
    ...
    # ---- Вспомогательные методы (безопасные) ----
