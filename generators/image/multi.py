...
class MultiImageGenerator(ImageGenerator):
    ...
    def generate(self, prompt: str, is_announce: bool = False, title: str = "", subtitle: str = "", cta: str = "") -> Optional[bytes]:
        ...
        # Если ни один генератор не дал результат – создаём баннер-заглушку
        if not image_bytes:
            logger.warning("Все генераторы не дали результат, создаём баннер-заглушку")
            try:
                # Берём тему и создаём нормальный баннер через BannerGenerator
                if is_announce:
                ...
                # Крайний случай – простая заглушка
