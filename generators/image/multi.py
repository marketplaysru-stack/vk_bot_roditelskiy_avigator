"""generators/image/multi.py – GenAPI с параметрами, резерв Picsum"""
import logging
...
class MultiImageGenerator(ImageGenerator):
    ...
    def generate(self, prompt: str, is_announce: bool = False, title: str = "", subtitle: str = "", cta: str = "") -> Optional[bytes]:
        ...
        try:
            ...
            # Передаём параметры как в примере
            result = self.genapi.generate(
                detailed_prompt,
                model="medium",
                aspect_ratio="9:16",          # вертикальный формат для постов
                creativity="medium",
                moodboards=[{"id": "1e51738c-7413-469e-93b6-ad50db460a1f", "strength": 1}]
            )
            if result:
        ...
        # Резерв – Picsum с текстом
        logger.warning("GenAPI не сработал, используем Picsum с текстом")
        try:
